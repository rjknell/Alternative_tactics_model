...
# The model is coded as a function called "ART_sim". To run it, just paste the entire text file into the R console, press enter and then type ART_sim(). The arguments to the function can be used to change the model parameters, so simulation_SS(K.cap = 500) will run the model for a carrying capacity of 500. All the model arguments are detailed below.
...
#type: the ART is fixed or simultaneous sensu Taborsky, M., Oliveira, R. F. & Brockmann, H. J. The evolution of alternative reproductive tactics: concepts and questions. Alternative reproductive tactics: an integrative approach 1, 21 (2008)
...
#ART_success: determines the likelihood of minor males acquiring matings
...
# Reproduction ----
    ...
    # If there are majors and mating is not random
    	...
    	#If type == fixed then only sample from majors
    	...
    	#If type == "simultaneous" then sample from majors & minors
...
succesful_minors <-
  ifelse(
    runif(no_females) < ART_success * prop_minors,
...
# Offspring characteristics ----
      ...
      new_features <-
        ...
        # if (sum(new_features$newborns > 0)) {
          ...
          # Select one male (major or minor) to be the father for each offspring. When a female has mated with a major and a minor there is am equal chance of either being the father. 
          ...
          new_features$male <- ifelse(new_features$minor == 0 | prob_minor <= 0.5, new_features$major, new_features$minor)
